...
import { CodingAttemptOptions, NS } from "Bitburner";
import { find } from "./find";

export async function main(ns: NS) {
	const res = find(ns, ['.cct'])
	if (!res.length) {
		ns.tprint("No contracts!")
		return
	}
	
	for (const {host, files} of res) {
		files.forEach(f => solveContract(ns, host, f, 0))
	}
	
	ns.tprint(`Solved ${res.length} contracts`)
}

export function solveContract(ns: NS, host: string, filename: string, logLevel=0) {
	...
	const opts: CodingAttemptOptions = {
	    returnReward: true
    }
...
function sanitizeParentheses(ns: NS, data) {
...
function findSanitized(ns: NS, s, pos, context) {
...
/** @param {NS} ns **/
function mergeOverlappingIntervals(ns: NS, data) {
...
/** @param {NS} ns **/
function generateIpAddresses(ns: NS, data) {
	return parseIpNum(ns, data, []);
}

/** @param {String} s
 * @Param {Array} parts**/
function parseIpNum(ns: NS, s, parts) {
	if (parts.length == 4) {
		if (s.length == 0) {
...
/** @param {NS} ns **/
function uniquePathInGrid2(ns: NS, data) {
...
/** @param {NS} ns **/
function subarrayWithMaxSum(ns: NS, data) {
...
/** @param {NS} ns **/
function arrayJumpingGame(ns: NS, data) {
...
/** @param {NS} ns **/
function algorithmicStockTrader1(ns: NS, data) {
	if (data.length == 0) {
		return 0;
	}
...
/** @param {NS} ns **/
function algorithmicStockTrader2(ns: NS, data) {
	if (data.length == 0) {
		return 0;
	}
	var chances = findProfitChances(data);
...
/** @param {NS} ns **/
function algorithmicStockTrader3(ns: NS, data) {
	if (data.length == 0) {
		return 0;
	}
	var chances = findProfitChances(data);
...
/** @param {NS} ns **/
function algorithmicStockTrader4(ns: NS, data) {
	if (data[1].length == 0) {
		return 0;
	}
...
/** @param {NS} ns **/
function minPathSumInTriangle(ns: NS, data) {
...
/** @param {NS} ns **/
function largestPrimeFactor(ns: NS, data) {
	var factor = 0;
	var k = data;
	var rk = Math.sqrt(k);
	for (var i = 2; i < rk;) {
...
function uniquePathInGrid1(ns: NS, data) {
	var a = data[0];
	var b = data[1];
	if (a > b) {
		a = data[1];
...
function spiralizeMatrix(ns: NS, data) {
	var s = 0;
	var m = [];
...
function totalWayToSum(ns: NS, data) {
	var cache = {};
	var n = data;
